#!/usr/bin/env python3
"""
score-validity.py — grade the worldbuilding-critic against the validity set, in BOTH directions:
let-an-unsound-system-through (false-pass) and reject-a-sound-one (false-fail, e.g. wrongly failing
a deliberately soft magic). Mirrors the quality-critic harness; the scorer is domain-agnostic.

Each item has fail_passage (should FAIL) and pass_passage (should PASS), tagged by `kind` (the
rubric dimension under test). You run the critic BLIND on each passage, record its PASS/FAIL on the
target dimension into a verdicts JSON, then this scorer compares against the gold labels.

Headline metrics:
  - UNSOUND-TOLERANCE = false-pass rate (passed something that should fail) — lower is better.
  - SOFT-MAGIC CONFORMITY = false-fail rate on soft-magic-trap (rejected a sound soft system).
  - overall false-pass / false-fail rates, plus a per-kind breakdown.
...
def load_set(path: Path) -> dict:
    ...
    Uses PyYAML if present; otherwise a tolerant fallback scanning the known structure.
    """
...
def main() -> int:
    ...
    ap.add_argument("--max-conformity", type=float, default=0.0,
                    help="max tolerated soft-magic-conformity rate; hard gate, independent of --max-false-fail (default 0.0)")
    args = ap.parse_args()
    ...
    for sid, g in sorted(gold.items()):
        ...
        if vf not in VALID_TOKENS or vp not in VALID_TOKENS:
            ...
            continue
        bk["scored"] += 1
        fp = vf == "PASS"   # passed something that should FAIL → too lenient (unsound let through)
        ff = vp == "FAIL"   # failed something that should PASS → too harsh (rejected a sound system)
        if fp:
            false_pass += 1
            bk["false_pass"] += 1
        if ff:
    ...
    soft = by_kind.get("soft-magic-trap")
    soft_scored = soft["scored"] if soft else 0
    soft_conformity = (soft["false_fail"] / soft_scored) if soft_scored else None

    print("\n=== worldbuilding-critic VALIDITY REPORT ===\n")
    print(f"{'ID':<28}{'KIND':<20}{'DIMENSION':<30}{'FAIL-PSG':<10}{'PASS-PSG':<10}OUTCOME")
    print("-" * 112)
    for sid, kind, dim, vf, vp, tag in rows:
        print(f"{sid:<28}{kind:<20}{dim:<30}{vf:<10}{vp:<10}{tag}")

    print("\n--- by control kind ---")
    for kind, k in sorted(by_kind.items()):
        print(f"  {kind:<22} valid {k['valid']}/{k['scored']}   false-pass {k['false_pass']}/{k['scored']}   false-fail {k['false_fail']}/{k['scored']}")

    excluded = missing + unusable
    print("\n--- headline metrics ---")
    print(f"  items scored:          {scored}/{n}" + (f"  ({missing} missing, {unusable} unusable)" if excluded else ""))
    print(f"  soundness accuracy:    {accuracy:.0%}  (both calls correct)")
    print(f"  UNSOUND-TOLERANCE:     {fp_rate:.0%}  (passed an unsound system — lower is better)")
    if soft_conformity is not None:
        print(f"  SOFT-MAGIC CONFORMITY: {soft_conformity:.0%}  (failed a sound soft system — crushed a valid choice)")
    ...
    harness_pass = (
        ...
        and (soft_conformity is None or soft_conformity <= args.max_conformity)
    )
