...
name: scqa-formatter
description: >
  Shape an audited problem statement as Situation/Complication/Question/Answer per Minto.
  Use for peer audiences, document introductions, or any output where the reader needs
  the complication to earn the answer. Returns SCQA narrative with optional one-level
  supporting pyramid below the answer; selects among standard, SCRA, and Concern-priming
  variants based on audience skepticism.
metadata:
  author: nirav
  version: "1.0"
